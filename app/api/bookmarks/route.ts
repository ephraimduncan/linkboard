...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ message: "Bookmark added successfully", id: bookmark.id }, { status: 200, headers });
  } catch (error) {
    console.error("Error adding bookmark:", error);
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: "Invalid bookmark data", details: error.errors }, { status: 400, headers });
    }
